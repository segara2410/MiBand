...
import custom.freeyourgadget.MiBandApp.util.ImportExportSharedPreferences;
...
public class DbManagementActivity extends AbstractGBActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button liveActivityButton = findViewById(R.id.exportHeartStepsTable);
        liveActivityButton.setOnClickListener(v -> {
            exportLiveActivtyTable();
        });
    ...
    private void exportLiveActivtyTable() {
        try (DBHandler dbHandler = GBApplication.acquireDB()) {
            exportShared();
            DBHelper helper = new DBHelper(this);
            File dir = FileUtils.getExternalFilesDir();
            ...
            File file = new File(dir, "liveactivity.csv");
            try {
                file.createNewFile();
                ...
                while(cursor.moveToNext()) {
                    ...
                    csvWriter.writeNext(columnExported);
                }
                csvWriter.close();
                cursor.close();

                GB.toast(this, getString(R.string.dbmanagementactivity_exported_to, file.getAbsolutePath()), Toast.LENGTH_LONG, GB.INFO);
            } catch (Exception e) {
                GB.toast(this, getString(R.string.dbmanagementactivity_error_exporting_db, e.getMessage()), Toast.LENGTH_LONG, GB.ERROR, e);
            }
